...
export class TestComponent implements OnInit {
    ...
    foo() {
        ...
        //firstly we sorting our arr by uniques parentId, for saving time in future cycle
        //secondly we use cycle "for", for uniques padrentID array and nested "for" for tasks array,
        //next we compare current item from uniques array with current item from tasks array,
        //and if first item equal second, we call function findCurrentTask()
        //this function have two parametrs, first its TASK_ID and second its object 
        ...
        const uniques = [...new Set(this.tasks.map(item => item))]; //uniques parentID

        console.log(uniques)

        for(let i = 0; i<uniques.length ; i++){
            for(let j = 0; j<this.tasks.length; j++){
                if(uniques[i] === this.tasks[j].Task_Parent_ID ){
                    this.findCurrentTask(uniques[i], this.tasks[j])
                }
            }
        }
    ...
    findCurrentTask(i, j){ //i - parent id, j - child object
        for(let k=0; k<this.tasks.length; k++){            
            if(this.tasks[k].Task_ID === i ){
                if('Child_Nodes' in this.tasks[k]){
                    this.tasks[k].Child_Nodes.push(j)
                }
                else{
                    Object.assign(this.tasks[k], {Child_Nodes: [j]})
                }
                return
            }
        }
